...
var UserRels = struct {
	Organization       string
	CreatorInstruments string
	CreatorSounds      string
}{
	Organization:       "Organization",
	CreatorInstruments: "CreatorInstruments",
	CreatorSounds:      "CreatorSounds",
}

// userR is where relationships are stored.
type userR struct {
	Organization       *Organization
	CreatorInstruments InstrumentSlice
	CreatorSounds      SoundSlice
}
...
var (
	userColumns               = []string{"id", "name", "password", "email", "organization_id", "created_at", "updated_at"}
	userColumnsWithoutDefault = []string{"name", "password", "email", "organization_id", "created_at", "updated_at"}
...
func (o *User) CreatorInstruments(mods ...qm.QueryMod) instrumentQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"instruments\".\"creator_id\"=?", o.ID),
	)
	...
	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"instruments\".*"})
	}
...
func (o *User) CreatorSounds(mods ...qm.QueryMod) soundQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"sounds\".\"creator_id\"=?", o.ID),
	)
...
// LoadCreatorInstruments allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadCreatorInstruments(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`instruments`), qm.WhereIn(`creator_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load instruments")
	}

	var resultSlice []*Instrument
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice instruments")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on instruments")
	}
	if err = results.Err(); err != nil {
	...
	if singular {
		object.R.CreatorInstruments = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.CreatorID {
				local.R.CreatorInstruments = append(local.R.CreatorInstruments, foreign)
				if foreign.R == nil {
					foreign.R = &instrumentR{}
				}
...
// LoadCreatorSounds allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadCreatorSounds(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`sounds`), qm.WhereIn(`creator_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.CreatorSounds = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &soundR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.CreatorID {
				local.R.CreatorSounds = append(local.R.CreatorSounds, foreign)
				if foreign.R == nil {
					foreign.R = &soundR{}
				}
				foreign.R.Creator = local
...
// AddCreatorInstruments adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.CreatorInstruments.
...
func (o *User) AddCreatorInstruments(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Instrument) error {
	...
	if o.R == nil {
		o.R = &userR{
			CreatorInstruments: related,
		}
	} else {
		o.R.CreatorInstruments = append(o.R.CreatorInstruments, related...)
	}
...
// AddCreatorSounds adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.CreatorSounds.
// Sets related.R.Creator appropriately.
func (o *User) AddCreatorSounds(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Sound) error {
	...
	if o.R == nil {
		o.R = &userR{
			CreatorSounds: related,
		}
	} else {
		o.R.CreatorSounds = append(o.R.CreatorSounds, related...)
	}
